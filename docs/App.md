...
3. **Price feed — resolved, built differently than first researched, then again once more:** the originally-assumed Backpack `/api/v1/ticker` endpoint isn't what shipped. The real `GET /api/stocks` route (see Architecture.md) sources live prices from Jupiter's Price API v3 (`https://lite-api.jup.ag/price/v3`, public, no-auth) and — as of the mainnet pass — the stock catalog itself from Jupiter's token API too (`tokens/v2/tag?query=verified`, filtered to `stocks`+`backpack` tags plus a shared-`freezeAuthority` check). Originally sourced the catalog from Sunrise's own listings API instead; that started requiring a non-self-serve API key, so it moved onto infrastructure the app already depends on. Polled client-side every 15s via `useLiveStocks`.
